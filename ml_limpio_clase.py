...
for filename in os.listdir(csv_directory):
    if filename.endswith(".csv"):
        ...
        if "flexion" in filename.lower():
            labels = np.zeros(shape=(windows.shape[0],))  # Etiquetas 0
        elif "extension" in filename.lower():
            labels = np.ones(shape=(windows.shape[0],))  # Etiquetas 1
        else:
...
# Entrenar el modelo
modelo.fit(train_data, train_labels, validation_data=(test_data, test_labels), epochs=10)

# Guardar el modelo
modelo.save("C:/Users/Diego Castro/Documents/Uvigo/Uvigo/4º/TFG/MLsensor/modelo_lstm_alfa.h5")
